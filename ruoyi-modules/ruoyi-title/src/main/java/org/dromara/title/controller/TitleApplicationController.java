...
public class TitleApplicationController {
    ...
    @PostMapping public R<?> create(@RequestBody Map<String,Object> body) { return R.ok(service.createDraft(Long.valueOf(body.get("applicantId").toString()), Long.valueOf(body.get("batchId").toString()), body)); }
    ...
    @PostMapping("/{id}/review") public R<?> review(@PathVariable Long id, @RequestBody Map<String,Object> body) { return R.ok(service.review(id, String.valueOf(body.get("action")), Long.valueOf(body.get("reviewerId").toString()), (String) body.get("reason"), body.get("correctionHours") == null ? null : Long.valueOf(body.get("correctionHours").toString()))); }
    @GetMapping("/{id}") public R<?> get(@PathVariable Long id) { return R.ok(service.get(id)); }
    @GetMapping("/{id}/audit-events") public R<List<TitleEvaluationService.AuditEvent>> audit(@PathVariable Long id) { return R.ok(service.auditEvents(id)); }
}
